...
> ⚠️ **`GET /api/v1/todo`가 이 규칙의 유일한 예외다** (2026-08-13). 조회 API인데 그날 예보가 없으면 `404 SKIN_FORECAST_NOT_FOUND`를 낸다 — 이 엔드포인트가 **목록 생성을 겸해서** 예보가 없으면 만들 목록 자체가 없기 때문이다([api.md](api.md) §2.4).
>
> **위 표의 "일상적으로 발생한다"에 정확히 해당하는 자리이기도 하다** — 수면을 아직 올리지 않은 신규 사용자가 TODO 탭을 열면 매번 이 응답을 받는다. **새 조회 API를 만들 때 이 사례를 근거로 삼지 말 것.** 기본은 `200` + 상태 필드다.
