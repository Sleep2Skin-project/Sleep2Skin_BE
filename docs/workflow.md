...
PRD §7의 블로커는 코드로 결정할 수 없는 것들이다. 예보 산출(HOME-03) 구현 직전에 다음 2개를 반드시 확정한다:

- **B1** 가중합 스코어링 공식과 초기 가중치 (§9.1 임시 매핑 재확정 포함)
- **B7** 결측 밤(`HRV`·`RESTING_HEART_RATE` NULL)의 스코어링 처리

**B2(등급 컷오프)·B3(판정 오차 구간)은 2026-08-06 확정됐다** → PRD §10.
