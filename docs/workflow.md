...
**분담이 실제로 일어났고 전부 끝났다.** `report`는 팀원이 별도 브랜치(`feature/report-daily` → `feature/report-weekly-monthly`)로 맡아 완료했다.
...
**실제로 겪은 것 — 인수인계에서 어긋난 자리는 Repository가 아니라 문서였다.** `report`가 `SleepSessionRepository`·`SkinForecastRepository`를 직접 읽는데 시그니처는 문제가 없었고, 대신 **기능 PR 4개가 `docs/`를 한 줄도 고치지 않아** 명세와 코드가 갈렸다(월간 파라미터 `yearMonth` → 실제 `baseDate` 등).
...
**헬스체크는 재시도 루프다.** 고정 `sleep`은 짧으면 멀쩡한 배포를 실패로 잡고 길면 실패를 늦게 안다 — 첫 배포는 테이블을 전부 만들고 시딩까지 돈다.
...
| 테이블 전부 | Hibernate (`ddl-auto: update`) — 엔티티가 있는 만큼 만든다 |
...
> **빈 화면 자체는 2026-08-18 코드 변경으로 해결됐다** — 임계값이 후보를 거르지 않게 되어 개수가 항상 찬다. **그래도 이 SQL은 실행한다**: 임계값이 이제 **선발 순서**를 정하므로, 옛 값(`30~70`)이면 "지금 급한 항목"으로 분류되는 범위가 좁아 순서가 어긋난다.
...
**이번 배포에서는 `threshold` 상향 SQL도 함께 실행해야 한다** (위 "아직 실행하지 않은 것"). `30, 70`이 나오면 아직 안 돌린 것이다 — **에러가 아니라 추천 순서가 어긋날 뿐이라 알아채기 어렵다.** 개수는 2026-08-18 변경으로 어느 쪽이든 채워지므로 **화면만 봐서는 구분되지 않는다.**
