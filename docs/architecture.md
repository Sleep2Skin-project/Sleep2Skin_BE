...
GET /api/v1/todo?baseDate=
  ...
  없으면
   ...
   4. 제외    그날 예보가 null인 지표를 겨냥한 것   ← 우선순위를 계산할 수 없다
   5. 정렬    impact_score × (100 − 예보 점수) + verdictBonus
              동점은 id 오름차순
   6. 갈래    예보 점수 ≤ threshold 인 것을 앞으로   ← 거르지 않고 앞세운다
              나머지가 그 뒤를 잇는다
   7. 절단    AVOID 3 + DO 5                      → 8행 저장
```
...
**`threshold`는 후보를 거르지 않는다 — 어느 갈래에 설지만 정한다** (2026-08-18). 만족하는 것이 모자라면 미만족이 뒤를 채우므로 **절단 개수가 상한이 아니라 고정 개수**다. 8행보다 적어지는 경로는 4단계 하나뿐이다. 근거는 [erd.md](erd.md) §3.8.
