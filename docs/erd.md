...
**확정된 것은 "상태 전환 시에만 지급"까지다.** 같은 요청을 두 번 보내도 exp가 두 번 붙지 않게 하는 것이 요점이었고, 되돌리기(`DONE → PENDING`)를 어떻게 할지는 정해지지 않았다.

> ⚠️ **그래서 `exp`가 `DONE` 행 개수 × 10보다 커질 수 있다.** 현재 구현은 회수하지 않으면서 판정 기준이 "이번에 `DONE`이 됐는가"뿐이라 **`DONE → PENDING → DONE`을 반복하면 그때마다 `+10`이 붙는다.** 이 컬럼을 하나뿐인 출처로 둔 판단은 그대로지만, **`daily_todo`로 검산이 안 된다** — 위 문단이 기대한 성질이 아직 성립하지 않는다.
>
> 닫는 방법은 둘이다: 되돌릴 때 `−10`을 회수하거나, `daily_todo`에 "지급 완료" 표시를 남기거나. 후자는 컬럼이 늘고(§ "도메인 컬럼은 5개뿐이다"와 충돌) 전자는 "실수로 눌렀다 되돌렸더니 줄었다"로 읽힌다. **정해지기 전까지 `exp`를 정확한 값으로 취급하는 화면을 만들지 말 것.**
