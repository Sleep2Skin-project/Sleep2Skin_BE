...
> **MVP는 단계 미상 구간이 없다고 가정한다**([prd.md](prd.md) §2). 그래도 이 컬럼은 유지한다 — 전제가 깨지는 순간 합이 어긋나는 것을 **확인할 수 있는 유일한 수단**이고, 나중에 단계 미상을 제대로 다루게 되면 `total − (deep+rem+core)`가 곧 미상 구간의 길이가 된다. 지금 빼면 그때 백필할 방법이 없다.
