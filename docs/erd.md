...
| 예보 적중률 (HOME-07·HOME-09) | 예보 vs 실측 조인 후 계산. **주간·월간 리포트에는 넣지 않는다** — 화면에 없어 범위에서 뺐다 (2026-08-15) |
| 수면 점수 (REP-02/06/08, exp 적립) | `sleep_session`에서 다시 계산 ([prd.md](prd.md) §10.8) — **컬럼이 아니다** |
| 수면 피처-지표 상관계수 (REP-07) | `sleep_session` × `skin_measurement` 기간 조회 후 계산 |
...
> 이 테이블을 쓰는 기능은 **여전히 REP-03 하나뿐이다** (구현 완료 2026-08-15). `GET /report/daily/timeline`이 `startTime` 오름차순으로 읽어 그대로 내보낸다 — **집계는 하지 않는다.** 정렬은 파생 쿼리(`OrderByStartTimeAsc`)가 SQL로 보장하므로 응답을 만들며 다시 정렬하지 않는다.
...
| `blackhead_detected` | BOOLEAN | **NULL 허용** | 블랙헤드 감지 여부 — 같음 (**2026-08-20 추가**) |
...
#### 감지 플래그 4종은 예보와 짝이 없다 (2026-08-16 추가 — [prd.md](prd.md) §7 L9 해소)

**종합 리포트(REP-10)의 "클리닉 필요" 항목을 만들기 위해 추가됐다.** LLM Vision이 매 실측마다 지표 3종과 함께 판정한다.

> **2026-08-20에 `blackhead_detected`가 늘어 3종 → 4종이 됐다.** 아래 성질은 그대로 적용된다 — **플래그가 늘어도 예보 쪽에는 아무 컬럼도 늘지 않는다**는 것이 이 절의 요점이고, 그 점이 실제로 지켜졌다(`skin_forecast` 무변경 · `personal_weight` 7행 유지).
...
> **새 행은 항상 네 값이 채워진다.** Structured Outputs가 strict 모드라 네 필드 전부 `required`이고, `SkinVisionScores`에서는 원시 `boolean`이다. **`NULL`은 과거 행에서만 나온다.**
>
> ⚠️ **도입 시점이 컬럼마다 다르므로 "과거 행"의 기준도 컬럼마다 다르다.** 2026-08-16~08-20 사이에 쌓인 행은 앞의 셋이 채워져 있고 `blackhead_detected`만 `NULL`이다. **행 하나 안에서 일부만 `NULL`인 것이 정상 상태다** — 플래그를 더 늘리면 같은 일이 또 생긴다.
...
| `threshold` | INT | NOT NULL | 해당 지표가 **이 값 이하**면 우선 선발 (2026-08-18부터 탈락 조건이 아니다 — 아래) |
...
**심각도를 `threshold`가 아니라 `100 − 점수`로 잡는다.** `threshold − 점수`로 하면 임계값이 발동 조건과 우선순위를 겸하게 된다. 기획자가 "이 항목이 잘 안 떠요" 하고 임계값을 올리는 순간 그 항목의 우선순위까지 조용히 올라간다. **임계값은 어느 갈래에 설지만, 심각도는 지표 점수만 결정한다** (갈래는 아래 2026-08-18 항목 참조).

**정렬은 카테고리별로 따로 돈다.** `AVOID` 상위 3 + `DO` 상위 5 = **하루 8행**.
...
#### 임계값은 후보를 거르지 않는다 — 정렬 1순위다 (2026-08-18)

**절단 개수가 상한이 아니라 고정 개수가 됐다.** `TodoScoringPolicy.selectTop`이 두 갈래로 채운다.
...
**`threshold` 상향(2026-08-17, 전 행 `+20`)의 후속이다.** `+20`으로도 컨디션이 좋은 날엔 `DO`가 4개·0개로 내려갔고, **화면이 그리는 칸 수가 날마다 달라졌다.** 값을 더 올리는 선택지가 있었지만 계속 올리면 결국 전 행이 `100`이 되어 컬럼이 무의미해진다 — **매칭 방식을 바꾸면 임계값이 "지금 급한 것부터"라는 의미로 살아남는다.**

- **`impact_score`도 정렬식도 건드리지 않았다.** 바뀐 것은 "거른다 → 앞세운다" 하나다
- **임계값을 만족하는 항목은 우선순위 점수가 낮아도 미만족 항목보다 항상 앞선다.** 두 갈래를 각각 정렬한 뒤 이어 붙이므로 갈래를 넘는 역전이 없다
- **여전히 8행보다 적을 수 있다** — 그날 예보가 `null`인 지표를 겨냥한 후보는 우선순위 자체를 계산할 수 없어 빠진다(위 §"구현은 SQL 경계를"). **이제 이것이 유일한 부족 경로다**
- 지키는 테스트는 `TodoScoringPolicyTest.모자라면_임계값_미만족으로_채운다`와 `TodoServiceTest.컨디션이_좋은_날에도_개수를_채운다`다
...
`src/main/resources/db/seed/action_master.sql`에 **지표 3종 × 카테고리 2종 × 4개 = 24행**이 들어 있다. 최소선(18~20)보다 여유가 있어, **세 지표의 예보가 모두 산출된 날이면 `DO` 5칸이 항상 찬다**(후보 12개 중 5개를 고른다).

> 2026-08-18 이전에는 **두 지표 이상이 임계값 아래여야** 5칸이 찼다. 임계값이 후보를 거르지 않게 되면서 이 조건이 사라졌다. 지금 5칸을 못 채우는 경우는 **예보가 `DARK_CIRCLE`만 있는 날**(후보 4개)뿐이다.
...
> **다음 날 값 상향만으로는 부족하다는 것이 드러났고**, 2026-08-18에 임계값이 후보를 거르지 않도록 매칭 방식을 바꿨다(위 §3.8 "임계값은 후보를 거르지 않는다"). **상향 자체는 유효하다** — 임계값이 이제 선발 순서를 정하기 때문이다. `action_master_raise_threshold.sql` 실행은 여전히 필요하다.

- **`impact_score`는 건드리지 않았다.** 임계값은 **어느 갈래에 설지만** 결정하고 심각도는 `100 − 점수`가 정하므로(아래), 임계값을 올려도 **갈래 안의 정렬 순서가 따라 움직이지 않는다.** 이 분리가 여기서 값을 했다
...
**행이 8개보다 적을 수 있다.** 그날 예보가 산출된 지표를 겨냥한 후보가 모자랄 때다(§3.8). **임계값 미달로는 더 이상 줄지 않는다** — 2026-08-18부터 미달 후보가 뒤를 채운다.

**후보가 0개면 `daily_todo` 행이 하나도 생기지 않아 다음 조회에서 추천 엔진이 다시 돈다** — "행 없음"과 "만들었지만 비었음"을 구분하지 않기 때문이다. 후보가 0개인 날은 애초에 처방할 것이 없는 날이라 다시 도는 비용이 문제되지 않는다.
...
>
> **한동안 맞지 않았다** (2026-08-15에 해소). `TodoService`가 `+5`가 아니라 `+10`을 주고 있었고 전체 완료 보너스는 구현조차 되지 않았다 — **이 검산식이 그 어긋남을 드러낸 유일한 장치였다.** 두 값 모두 범위가 정상이라 제약에도 테스트에도 걸리지 않았다. **적립 지점을 새로 만들면 이 식이 여전히 맞는지 확인한다.**
...
### ~~수면 목표값~~ — MVP 제외 (2026-08-14)

**어디에도 두지 않는다.** `users` 컬럼도, `ScoringPolicy` 상수도 아니다 — 값이 있으면 어딘가에서 쓰이게 된다.

목표를 설정하는 화면이 명세에 없고, 서버가 공통 권장치를 임의로 정하면 **근거 없는 숫자가 "목표 달성"이라는 판정문으로 나간다.** 리포트는 관측값을 그대로 보여준다 ([prd.md](prd.md) §4.4 · §7 B6).

> 사용자별 목표로 갔다면 `users`에 컬럼이 하나 늘었을 자리다. **빼기로 하면서 스키마가 그대로 남았다.**
...
> ✅ **예외였던 하나는 실제로 스키마를 건드렸다** (2026-08-16). REP-09 논의의 **셀피 실측 전용 지표 추가 안**이 채택되어 `skin_measurement`에 **컬럼이 셋** 늘었다(색소침착 하나가 아니라 여드름 흉터·구조적 노화까지 — §3.6). 점수가 아니라 **감지 여부 boolean**이라 예보 쪽에는 아무 컬럼도 늘지 않았다. **2026-08-20에 `blackhead_detected`가 같은 방식으로 하나 더 늘어 넷이 됐다.**
>
> ⚠️ **`ddl-auto: update`는 컬럼을 지우지 않는다는 점은 그대로다**(§5). 그래서 이 넷은 처음부터 **NULL 허용**으로 들어갔다 — 되돌릴 수 없는 변경이라면 최소한 기존 행을 오염시키지 않아야 한다.
