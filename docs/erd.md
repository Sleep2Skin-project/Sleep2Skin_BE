...
>
> ⚠️ **지금은 맞지 않는다** (2026-08-15). `TodoService`가 `+5`가 아니라 `+10`을 주고 있고 **전체 완료 보너스는 구현되지 않았다** ([prd.md](prd.md) §10.9 · [api.md](api.md) §4). **이 검산식이 그 어긋남을 드러내는 유일한 장치다** — 두 값 모두 범위가 정상이라 제약에도 테스트에도 걸리지 않는다.
