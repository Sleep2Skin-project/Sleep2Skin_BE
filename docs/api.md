...
**⚠️ 그날 예보가 없으면 `404 SKIN_FORECAST_NOT_FOUND`다.** 조회 API인데 4xx라 §1의 규칙과 어긋나 보이지만, 이 엔드포인트는 **목록 생성이라는 동작을 겸하고** 예보가 없으면 만들 목록 자체가 없다. 다만 **수면을 아직 올리지 않은 신규 사용자가 TODO 탭을 열면 일상적으로 이 응답을 받는다** — 앱은 이 코드를 에러 화면이 아니라 "먼저 수면을 동기화하세요" 안내로 처리한다.

| 코드 | `ErrorCode` | 언제 |
|---|---|---|
| `400` | `INVALID_INPUT` | `baseDate` 누락·형식 오류 |
| `404` | `USER_NOT_FOUND` | 없는 사용자 |
| `404` | `SKIN_FORECAST_NOT_FOUND` | 그날 예보가 없어 목록을 만들 수 없음 |
...
**exp는 `PENDING → DONE`으로 처음 바뀔 때만 10 지급된다.**

| 요청 | `expGained` |
|---|---|
| `PENDING` → `DONE` | `10` |
| `DONE` → `DONE` (중복 호출) | `0` |
| `DONE` → `PENDING` (되돌리기) | `0` — **회수하지 않는다** |
| `PENDING` → `DONE` (되돌린 뒤 다시 체크) | **`10`** — 아래 ⚠️ |

**확정된 것은 "상태 전환 시에만 지급"까지다.** 같은 요청을 두 번 보내도 exp가 두 번 붙지 않게 하는 것이 요점이었다. `totalExp`는 지급 이후 `users.exp`의 값이다.

> ⚠️ **되돌리기를 어떻게 할지는 정해지지 않았고, 현재 구현은 회수하지 않는다.** 판정 기준이 "이번에 `DONE`이 아니었다가 `DONE`이 됐는가"뿐이라 **`DONE → PENDING → DONE`을 반복하면 그때마다 `+10`이 붙는다.** 중복 호출(`DONE → DONE`)은 막히지만 껐다 켜는 것은 막히지 않는다.
>
> 회수하거나(되돌리면 `−10`), 그 `daily_todo` 행이 이미 지급받았는지를 기록해야 닫힌다. 후자는 컬럼이 하나 늘고 전자는 "실수로 눌렀다 되돌렸더니 줄었다"로 읽힌다 — **어느 쪽인지 정해야 한다**([prd.md](prd.md) §9.2).
