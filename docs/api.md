...
**`hitRate`는 판정 개수가 아니라 정확도 평균이다** (2026-08-19 교체). 지표마다 `|예보 − 실측|`을 로그 곡선으로 `0~100`에 대응시키고 그 평균을 낸다.

| 오차 | 0 | 3 | 5 | 10 | 15 | 20 | 30 | 50 | 100 |
|---|---|---|---|---|---|---|---|---|---|
| 정확도 | 100 | 90 | 85 | 75 | 68 | 61 | 52 | 39 | 20 |

- **`verdict`와 다른 축이다.** 판정은 ±5·±15로 잘린 라벨이고 적중률은 연속값이다 — **적중(`HIT`)이어도 `100`이 아니다**
- **`100`은 세 지표가 전부 정확히 맞아야 나오고, 완전히 빗나가도 `20` 밑으로 내려가지 않는다**
- 예전에는 `HIT` 개수 ÷ 판정 수여서 지표가 3개면 `0`·`33`·`67`·`100`만 나왔고, **오차 6점과 60점이 똑같이 "적중 아님"으로 뭉개졌다**
- 곡선 계수(`ScoringPolicy.ACCURACY_FLOOR`·`ACCURACY_SCALE`)는 **§10에 없는 임시값이다**

**평균낼 분모는 `verifications`의 길이다 — 3이 아니다.** 빈 지표를 0점으로 취급하면 존재하지 않는 오차가 적중률에 섞이고, 같은 값이 HOME-08의 학습 입력으로 들어가 **없던 값이 개인 가중치를 움직인다.** `verifications`는 비지 않는다 — `DARK_CIRCLE`은 예보가 빈 상태가 될 수 없기 때문이다([erd.md](erd.md) §3.5).
...
**3. 적중률 · 연속 검증 배너** (HOME-09) — 최근 검증 1건 + 직전 검증 1건 + 적중률 + 연속 검증 횟수.
...
{ "success": true,
  "data": {
    ...
    "baseDate": "2026-08-10",
    "summary": {                    // 빈 상태면 null
      "hitRate": 72,                // 누적 — 지금까지 모든 판정의 정확도 평균(%)
...
**`hitRate`는 누적이다** (2026-08-10 확정). 최근 1건만 쓰면 표본이 최대 3이라 하루마다 요동친다. 배너가 말하려는 것은 *"예보가 얼마나 믿을 만한가"* 이므로 표본이 쌓일수록 안정되는 쪽이 맞다. **그날치는 `latest.hitRate`에 따로 있다.**

**정확도 곡선은 `POST /skin/selfie`와 같다.** 누적은 판정 하나하나가 한 표이므로 **지표가 2개뿐이던 날은 자동으로 가벼워진다** — 날짜별 적중률을 다시 평균내지 않는다.

**`previous`는 화면의 "지난번 대비" 기준선이다** (2026-08-19 추가). 화면이 그날치 적중률 옆에 상승폭을 그리는데, 비교 대상이 응답에 없었다.

- **상승폭은 서버가 담지 않는다.** `latest.hitRate − previous.hitRate`를 앱이 뺀다 — 같은 응답 안의 두 필드로 나오는 값이라 서버가 함께 주면 **같은 사실을 말하는 자리가 둘이 되어 어긋난다**([conventions.md](conventions.md) §11의 `allCompleted`와 같은 이유)
- **`previous.baseDate`는 전날이 아니라 바로 앞 검증일이다.** 하루 걸러 검증했으면 이틀 전이다
- **검증이 1건뿐이면 `null`이다** — 비교할 대상이 없다. `0`을 주면 화면이 **없던 상승폭**을 그린다
- **누적 `hitRate`를 기준선으로 쓰지 않은 이유**: 누적에는 그날치가 이미 포함돼 있어 자기 자신과 비교하는 꼴이 되고, 검증 횟수가 적을수록 상승폭이 실제보다 눌린다
- ⚠️ **이 차이는 그날치끼리의 비교라 요동친다.** 표본이 최대 3이라 한 지표가 10점 더 빗나가는 것만으로 `5%p` 안팎이 움직인다. **"예보가 얼마나 믿을 만한가"는 여전히 누적이 말한다** — 둘을 같은 뜻으로 쓰지 말 것
- **조회가 늘지 않았다.** 누적 적중률이 어차피 검증한 날 전량(`findVerifiedDays`)을 훑으므로 두 번째 원소를 꺼낼 뿐이다. `skipped`를 만들지 않아 `sleep_session`도 읽지 않는다
