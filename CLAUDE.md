...
### 클리닉 트리아지 플래그는 예보 지표가 아니다 (2026-08-16 3종 · 2026-08-20 4종)

`skin_measurement`에 **`pigmentationDetected`·`acneScarDetected`·`agingDetected`·`blackheadDetected`** 네 컬럼이 늘었다. **지표 3종 고정 원칙을 깬 것이 아니다** — 예보에 대응 값이 없는 **실측 전용 감지 플래그**이고, 점수가 아니라 boolean이다.
...
- **⚠️ 도입 시점이 필드마다 달라 "도입 이전"의 기준도 다르다.** 2026-08-16~08-20 사이 실측 행은 앞의 셋이 채워져 있고 `blackheadDetected`만 `NULL`이다 — **행 하나 안에서 일부만 `NULL`인 것이 정상**이다
- **`SkinMetric` enum에 넣지 말 것.** 예보와 실측이 같은 세트라는 전제는 그 enum이 지키고 있다
- **플래그는 늘려도 되고 지표는 늘리면 안 된다.** 이 비대칭이 요점이다 — 플래그를 하나 더 붙일 때 드는 비용은 프롬프트 필드·컬럼·응답 필드 셋뿐이고, `SkinVisionScores`가 record라 **빠뜨린 자리는 컴파일 에러로 드러난다.** 반면 지표를 늘리면 예보·검증·학습이 전부 따라와야 한다
...
2. **클리닉 필요 3종** (L9) — 색소침착만이 아니라 **셋 전부**를 `skin_measurement`의 감지 boolean으로 추가했다(위 "클리닉 트리아지 플래그"). 예고대로 `skin` 도메인(엔티티 · `SkinVisionScores` · Vision 프롬프트 · 구조화 출력 스키마)이 함께 바뀌었고, **담당자 부재로 `report` 쪽에서 직접 구현하며 기존 지표 3종 경로가 그대로인지 diff로 확인**했다. **다음에 `skin`을 건드릴 때도 같은 확인을 거친다** — **2026-08-20 블랙헤드 추가가 그 첫 사례였고, 같은 경로를 그대로 복제해 넷째 플래그가 됐다**
