...
이 전제로 **단계 미상(`asleepUnspecified`) 처리와 부분 결측 비율 계산을 보류**했고, **B5(앱↔서버 단계 계약)도 보류**됐다 — 페이로드 규격은 우리가 정의해 통보하면 된다.

**전제가 깨져도 틀린 값이 나가지 않게 방어만 넣는다** — `deep == 0 && rem == 0`이면 `BARRIER`를 빈 상태로 (§10.6). **0점과 결측은 다르다.** 그대로 계산하면 장벽이 "위험"으로 나가는데, 이건 없는 위험을 경고하는 것이다.
